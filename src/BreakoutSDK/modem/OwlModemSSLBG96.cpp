...
bool OwlModemSSLBG96::setDeviceCert(str cert, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_cert.pem\",2", 1 * 1000, &ssl_response,
                                    SSL_RESPONSE_BUFFER_SIZE) == AT_Result_Code__OK) {
      ...
      OwlModemAT::filterResponse(s_qfopen, &ssl_response);
      long int fdesc = str_to_long_int(ssl_response, 10);
      char closebuf[32];
      snprintf(closebuf, 32, "AT+QFCLOSE=%d", (int)fdesc);
      if (atModem_->doCommandBlocking(closebuf, 1 * 1000, &ssl_response, SSL_RESPONSE_BUFFER_SIZE) !=
          AT_Result_Code__OK) {
  ...
  if (write_cert) {
    atModem_->doCommandBlocking("AT+QFDEL=\"ssl_cert.pem\"", 1 * 1000, nullptr,
                                0);  // ignore the result, which will be error if file does not exist
  ...
  if (atModem_->doCommandBlocking("AT+QSSLCFG=\"clientcert\",0,\"UFS:ssl_cert.pem\"", 1 * 1000, nullptr, 0) !=
      AT_Result_Code__OK) {
    return false;
  }
...
bool OwlModemSSLBG96::setDevicePkey(str pkey, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_pkey.pem\",2", 1 * 1000, &ssl_response,
                                    SSL_RESPONSE_BUFFER_SIZE) == AT_Result_Code__OK) {
      // Just check if the file exists. A better solution would be to read out the file and compare, or use QFCRC, which
      // is only present in the newest firmware revisions
      OwlModemAT::filterResponse(s_qfopen, &ssl_response);
  ...
  if (write_pkey) {
    atModem_->doCommandBlocking("AT+QFDEL=\"ssl_pkey.pem\"", 1 * 1000, nullptr,
                                0);  // ignore the result, which will be error if file does not exist
  ...
  if (atModem_->doCommandBlocking("AT+QSSLCFG=\"clientkey\",0,\"UFS:ssl_pkey.pem\"", 1 * 1000, nullptr, 0) !=
      AT_Result_Code__OK) {
    return false;
  }
...
bool OwlModemSSLBG96::setServerCA(str ca, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_cacert.pem\",2", 1 * 1000, &ssl_response,
                                    SSL_RESPONSE_BUFFER_SIZE) == AT_Result_Code__OK) {
      // Just check if the file exists. A better solution would be to read out the file and compare, or use QFCRC, which
      // is only present in the newest firmware revisions
      OwlModemAT::filterResponse(s_qfopen, &ssl_response);
  ...
  if (write_ca) {
    atModem_->doCommandBlocking("AT+QFDEL=\"ssl_cacert.pem\"", 1 * 1000, nullptr,
                                0);  // ignore the result, which will be error if file does not exist
  ...
  if (atModem_->doCommandBlocking("AT+QSSLCFG=\"cacert\",0,\"UFS:ssl_cacert.pem\"", 1 * 1000, nullptr, 0) !=
      AT_Result_Code__OK) {
    return false;
  }
