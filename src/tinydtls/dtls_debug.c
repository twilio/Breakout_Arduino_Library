...
#ifdef WITH_ARDUINO
  owl_log_set_level((unsigned char) level);
#else
  maxlog = level;
#endif
}
...
#warning "inet_ntop() not available, network addresses will not be included in debug output"
  ...
  switch (addr->size) {
    case IP_Address__empty:
      ret_len = 5 >= len ? 5 : len;
      memcpy(buf, "empty", ret_len);
      return ret_len;
      break;
    case IP_Address__IPv4:
      ret_len = snprintf(buf, len, "%u.%u.%u.%u", addr->addr.ipv4[0],
                         addr->addr.ipv4[1], addr->addr.ipv4[2],
                         addr->addr.ipv4[3]);
      return ret_len >= 0 ? ret_len : 0;
      break;
    case IP_Address__IPv6:
      ret_len = snprintf(
          buf, len,
          "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%"
          "02x%02x:",
          addr->addr.ipv6[0], addr->addr.ipv6[1], addr->addr.ipv6[2],
          addr->addr.ipv6[3], addr->addr.ipv6[4], addr->addr.ipv6[5],
          addr->addr.ipv6[6], addr->addr.ipv6[7], addr->addr.ipv6[8],
          addr->addr.ipv6[9], addr->addr.ipv6[10], addr->addr.ipv6[11],
          addr->addr.ipv6[12], addr->addr.ipv6[13], addr->addr.ipv6[14],
          addr->addr.ipv6[15]);
      return ret_len >= 0 ? ret_len : 0;
      break;
    default:
      ret_len = 7 >= len ? 7 : len;
      memcpy(buf, "invalid", ret_len);
      return ret_len;
      break;
  }
...
dtls_dsrv_hexdump_log(log_t level, const char *name, const unsigned char *buf,
  ...
  if (extend) {
    dsrv_log(level, "%s: (%u bytes):\r\n", name, length);
    while (length--) {
      if (n % 16 == 0) len += snprintf(tmp + len, 64 - len, "%04X  -  ", n);

      len += snprintf(tmp + len, 64 - len, "%02X ", *buf++);
    ...
    dsrv_log(level, "%s: (%u bytes): ", name, length);
    len = 0;
    while (length--) {
      len += snprintf(tmp + len, 64 - len, "%02X", *buf++);
      if (len >= 64) {
        dsrv_loge(level, "%.*s", len, tmp);
        len = 0;
      }
    }
    dsrv_loge(level, "%.*s\r\n", len, tmp);
