...
dtls_set_record_header(uint8 type, dtls_security_parameters_t *security,
                       uint8 *buf, netq_t *node) {
  dtls_int_to_uint8(buf, type);
  buf += sizeof(uint8);

  /* Retransmissions should be identical, so re-using the old values */
  if (node) {
    dtls_int_to_uint16(buf, node->version);
    buf += sizeof(uint16ptr);

    dtls_int_to_uint16(buf, node->epoch);
    buf += sizeof(uint16ptr);

    dtls_int_to_uint48(buf, node->rseq);
    buf += sizeof(uint48ptr);
  } else if (security) {
    ...
    memset(buf, 0, sizeof(uint16ptr) + sizeof(uint48ptr));
    buf += sizeof(uint16ptr) + sizeof(uint48ptr);
  }

  memset(buf, 0, sizeof(uint16ptr));
  return buf + sizeof(uint16ptr);
}
...
static int verify_ext_eliptic_curves(uint8 *data, size_t data_length) {
  ...
  data += sizeof(uint16ptr);
  if (i + sizeof(uint16ptr) != data_length) {
    dtls_warn("the list of the supported elliptic curves should be tls extension length - 2\r\n");
    return dtls_alert_fatal_create(DTLS_ALERT_HANDSHAKE_FAILURE);
  }

  for (i = data_length - sizeof(uint16ptr); i > 0; i -= sizeof(uint16ptr)) {
			 ...
			 uint8 *data, size_t data_length, int client_hello)
{
  ...
  if (data_length < sizeof(uint16ptr)) {
  ...
  while (data_length) {
    if (data_length < sizeof(uint16ptr) * 2)
      goto error;
...
dtls_update_parameters(dtls_context_t *ctx,
  ...
  data += DTLS_HS_LENGTH + sizeof(uint16ptr);
  data_length -= DTLS_HS_LENGTH + sizeof(uint16ptr);
  ...
  i = dtls_uint16_to_int(data);
  if (data_length < i + sizeof(uint16ptr)) {
  ...
  data += sizeof(uint16ptr);
  data_length -= sizeof(uint16ptr) + i;
...
dtls_prepare_record(dtls_peer_t *peer, dtls_security_parameters_t *security,
		    ...
		    uint8 *sendbuf, size_t *rlen, netq_t *node) {
  uint8 *p, *start;
  int res;
  unsigned int i;
  ...
  p = dtls_set_record_header(type, security, sendbuf, node);
  start = p;
		...
		size_t buf_len_array[], size_t buf_array_len)
{
  ...
  if ((type == DTLS_CT_HANDSHAKE && buf_array[0][0] != DTLS_HT_HELLO_VERIFY_REQUEST) ||
    ...
    if (n) {
      dtls_tick_t now;
      dtls_ticks(&now);
      n->t = now + DTLS_FIRST_RETRANSMIT_SECONDS * CLOCK_SECOND;
      n->retransmit_cnt = 0;
      n->timeout = DTLS_FIRST_RETRANSMIT_SECONDS * CLOCK_SECOND;
      n->peer = peer;
      n->version = dtls_uint16_to_int(DTLS_RECORD_HEADER(sendbuf)->version);
      n->epoch = dtls_uint16_to_int(DTLS_RECORD_HEADER(sendbuf)->epoch);
      n->rseq = dtls_uint16_to_int(DTLS_RECORD_HEADER(sendbuf)->sequence_number);
      n->type = type;
      n->length = 0;
      for (i = 0; i < buf_array_len; i++) {
...
dtls_send_client_key_exchange(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  case TLS_PSK_WITH_AES_128_CCM_8: {
    ...
    len = CALL(ctx, get_psk_info, &peer->session, DTLS_PSK_IDENTITY,
	       ...
	       buf + sizeof(uint16ptr),
	       min(sizeof(buf) - sizeof(uint16ptr),
    ...
    if (len + sizeof(uint16ptr) > DTLS_CKXEC_LENGTH) {
    ...
    memcpy(handshake->keyx.psk.identity, p + sizeof(uint16ptr), len);
...
dtls_send_client_hello(dtls_context_t *ctx, dtls_peer_t *peer,
  ...
  if (cookie_length == 0) {
    ...
    dtls_prng(handshake->tmp.random.client + sizeof(uint32ptr),
         DTLS_RANDOM_LENGTH - sizeof(uint32ptr));
  }
		      ...
		      uint8 *data, size_t data_length)
{
  ...
  data += sizeof(uint16ptr);	      /* skip version field */
  data_length -= sizeof(uint16ptr);
			      ...
			      uint8 *data, size_t data_length)
{
  ...
  if (len != data_length - DTLS_HS_LENGTH - sizeof(uint16ptr)) {
...
dtls_handle_message(dtls_context_t *ctx,
  ...
  dtls_debug_hexdump("Rx-message", msg, msglen);
