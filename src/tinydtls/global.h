...
/* Renamed the types with prefix ptr, because uint16 is confusing on Arduino, where libmapple redefines it */

typedef unsigned char uint8;
typedef unsigned char uint16ptr[2];
typedef unsigned char uint24ptr[3];
typedef unsigned char uint32ptr[4];
typedef unsigned char uint48ptr[6];
...
#ifndef DTLS_FIRST_RETRANSMIT_SECONDS
#ifdef WITH_ARDUINO
#define DTLS_FIRST_RETRANSMIT_SECONDS 5 /* Setting to 5, because of NB-IoT RTT + Twilio Breakout service. */
#else
#define DTLS_FIRST_RETRANSMIT_SECONDS 2
#endif
#endif
...
#ifdef WITH_ARDUINO
#define DTLS_DEFAULT_MAX_RETRANSMIT 4  /* Setting to 4, for alignment with Twilio Breakout service. */
#else
