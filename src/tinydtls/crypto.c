...
//TODO - Investigate if this is the non-volatile storage and it is enough to just manage these functions in order
// to provide persistence of sessions. For now, doing a standard malloc
void crypto_init(void) {
}
...
dtls_mac(dtls_hmac_context_t *hmac_ctx, 
  ...
  assert(hmac_ctx);
  dtls_hmac_update(hmac_ctx, record +3, sizeof(uint16ptr) + sizeof(uint48ptr));
...
dtls_psk_pre_master_secret(unsigned char *key, size_t keylen,
  ...
  if (result_len < (2 * (sizeof(uint16ptr) + keylen))) {
    return -1;
  }
  ...
  memcpy(p, result, sizeof(uint16ptr));
  p += sizeof(uint16ptr);
  
  memcpy(p, key, keylen);

  return 2 * (sizeof(uint16ptr) + keylen);
}
#endif /* DTLS_PSK */
